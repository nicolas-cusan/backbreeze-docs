...
Backbreeze's configuration API is based on this principle. To configure a Backbreeze you need to overwitte it by creating a variable with the same name in your project:
...
Each CSS property class has two configuration variables, one that holds the property's [**values**](#values), and one that holds [**configuration**](#configuration) for that property (e.g. name, breakpoints, pseudo states), both variables are Sass maps.
